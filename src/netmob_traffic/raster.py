...
def compute_zonal_statistics_traffic_raster(city: City, traffic_raster: xr.DataArray, vectors: gpd.GeoDataFrame, coverage_threshold: float) -> pd.DataFrame:
    ...
    zonal_stats = [_get_zonal_statistics(c=city, s=s, t=t, raster=traffic_raster.sel(service=s, time=t), vectors=vectors) for s, t in iter_service_time]
    zonal_stats = pd.concat(zonal_stats)
    return zonal_stats


def _get_zonal_statistics(c: City, s: str, t: time, raster: xr.DataArray, vectors: gpd.GeoDataFrame) -> pd.DataFrame:
    zonal_stats = raster_utils.extract_zonal_stats(raster=raster, no_data=np.nan, vector=vectors)
    zonal_stats['service'] = s
    zonal_stats['time'] = t
    zonal_stats['city'] = c.value
    zonal_stats.rename(columns={'mean_raster_value': 'traffic'}, inplace=True)
    zonal_stats = zonal_stats[['idcar_200m', 'city', 'service', 'time', 'traffic']].copy()
    return zonal_stats
